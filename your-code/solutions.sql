...
SELECT authors.au_id AS AUTHOR_ID, authors.au_lname AS LAST_NAME, authors.au_fname AS FIRST_NAME, ((titles.ytd_sales * titles.royalty)*(titleauthor.royaltyper/100))+(titles.advance*(titleauthor.royaltyper/100)) AS PROFIT
FROM publications.authors
INNER JOIN publications.titleauthor
ON authors.au_id = titleauthor.au_id
INNER JOIN publications.titles
